...
describe('Plan A sealed groups', () => {
    it('packs contiguous completed tools, and seals them', () => {
        const blocks: ChatBlock[] = [
            makeToolBlock('a', 'Read', { file_path: 'a.ts' }),
            makeToolBlock('b', 'Bash', { command: 'ls' }),
            makeToolBlock('c', 'Read', { file_path: 'c.ts' }),
        ]
        const visible = buildVisibleChatBlocks(blocks)
        expect(isToolGroupBlock(visible[0])).toBe(true)
        if (!isToolGroupBlock(visible[0])) throw new Error('expected group')
        expect(visible[0].sealed).toBe(true)
        expect(visible[0].remoteItems).toBe(false)
        expect(visible[0].tools.map((t) => t.id)).toEqual(['a', 'b', 'c'])
    })

    it('does not grow a previously sealed group when new tools append', () => {
        const firstBatch: ChatBlock[] = [
            makeToolBlock('a', 'Read', { file_path: 'a.ts' }),
            makeToolBlock('b', 'Bash', { command: 'ls' }),
        ]
        const previous = buildVisibleChatBlocks(firstBatch)
        expect(isToolGroupBlock(previous[0])).toBe(true)
        if (!isToolGroupBlock(previous[0])) throw new Error('expected group')
        expect(previous[0].sealed).toBe(true)

        const nextBlocks: ChatBlock[] = [
            ...firstBatch,
            makeToolBlock('c', 'Read', { file_path: 'c.ts' }),
            makeToolBlock('d', 'Bash', { command: 'pwd' }),
        ]
        const next = buildVisibleChatBlocks(nextBlocks, {
            previousGroups: previous.filter(isToolGroupBlock)
        })
        const groups = next.filter(isToolGroupBlock)
        expect(groups).toHaveLength(2)
        expect(groups[0]?.id).toBe(previous[0].id)
        expect(groups[0]?.tools.map((t) => t.id)).toEqual(['a', 'b'])
        expect(groups[1]?.tools.map((t) => t.id)).toEqual(['c', 'd'])
        expect(groups[1]?.sealed).toBe(true)
    })

    it('maps remote summary blocks to sealed remote groups', () => {
        const remote = {
            kind: 'tool-group-remote' as const,
            id: 'tool-group:t1',
            createdAt: 10,
            firstSeq: 1,
            lastSeq: 9,
            summary: {
                totalTools: 3,
                countsByKind: { read: 2, search: 0, command: 1, mutation: 0, web: 0, other: 0 },
                fileTargets: ['a.ts'],
                commandTargets: ['ls'],
                searchTargets: [],
                urlTargets: [],
            ...
            toolsPreview: [
                { id: 't1', name: 'Read', state: 'completed' },
                { id: 't2', name: 'Bash', state: 'completed' },
                { id: 't3', name: 'Read', state: 'completed' },
            ],
        }
        const visible = buildVisibleChatBlocks([remote as unknown as ChatBlock])
        expect(isToolGroupBlock(visible[0])).toBe(true)
        if (!isToolGroupBlock(visible[0])) throw new Error('expected group')
        expect(visible[0].id).toBe('tool-group:t1')
        expect(visible[0].sealed).toBe(true)
        expect(visible[0].remoteItems).toBe(true)
        expect(visible[0].tools).toEqual([])
        expect(visible[0].summary.totalTools).toBe(3)
    })
})
