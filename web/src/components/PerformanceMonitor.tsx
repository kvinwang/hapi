...
export function PerformanceMonitor() {
    ...
    const clampPosition = useCallback((x: number, y: number) => ({
        x: Math.min(Math.max(8, x), Math.max(8, window.innerWidth - (collapsed ? 68 : 72))),
    ...
    return createPortal(
        <aside data-performance-monitor style={{ left: position.x, top: position.y }} className="fixed z-[100] w-16 overflow-hidden rounded-md bg-black/90 font-mono text-[9px] tabular-nums text-white shadow-xl">
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-0.5 gap-y-0.5 whitespace-nowrap px-1 py-1.5 leading-4">
                ...
                <span className="text-white/55">p/m</span><span className="text-right">{snapshot.p95FrameMs}/{snapshot.maxFrameMs}</span>
