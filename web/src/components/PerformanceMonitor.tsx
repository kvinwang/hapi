...
export function PerformanceMonitor() {
    ...
    const clampPosition = useCallback((x: number, y: number) => ({
        x: Math.min(Math.max(8, x), Math.max(8, window.innerWidth - (collapsed ? 68 : 84))),
    ...
    return createPortal(
        <aside data-performance-monitor style={{ left: position.x, top: position.y }} className="fixed z-[100] w-[76px] overflow-hidden rounded-md bg-black/90 font-mono text-[10px] tabular-nums text-white shadow-xl">
