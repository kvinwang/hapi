...
function getSessionPatch(value: unknown): SessionPatch | null {
    ...
    if (typeof value.effortMode === 'string') {
        patch.effortMode = value.effortMode as Session['effortMode']
        hasKnownPatch = true
    }
...
function hasUnknownSessionPatchKeys(value: unknown): boolean {
    ...
    const knownKeys = new Set(['active', 'thinking', 'activeAt', 'updatedAt', 'permissionMode', 'modelMode', 'effortMode'])
...
export function useSSE(options: {
        ...
        const patchSessionSummary = (sessionId: string, patch: SessionPatch): boolean => {
            ...
            queryClient.setQueryData<SessionsResponse | undefined>(queryKeys.sessions, (previous) => {
                ...
                const nextSummary: SessionSummary = {
                    ...
                    effortMode: patch.effortMode ?? current.effortMode
                }
    ...
    }, [options.baseUrl, options.enabled, subscriptionKey, queryClient, reconnectNonce])
