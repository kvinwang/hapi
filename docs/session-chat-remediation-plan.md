...
| 2026-07-25 | Load-more settle | `9abb8c21` | Keep load-older anchors alive until height stabilizes; refresh baselines after each restore so late async layout does not jump. | Focused scroll-position tests; web typecheck | Passed | Retained |
| 2026-07-26 | Production rollout | `fb743aa0` | The tool-group work was only ever running in the isolated validation deployment; production still served a hub binary built before the feature, so no page ever carried a compacted group. | Compared the production bundle hash and embedded client with the local build; rebuilt `build:single-exe`; swapped `/home/kvin/.local/bin/hapi-hub` and restarted `hapi-search-hapi-hub-1` through a detached script; re-measured on a read-only copy of the reported session | Production now serves the new client; the reported session compacts 50 runs / 434 tools, 3302 KB -> 1423 KB (57% less) | Retained |
