...
export class Query implements AsyncIterableIterator<SDKMessage> {
    ...
    async getUsage(): Promise<SDKControlResponse['response']> {
        if (!this.childStdin) {
            throw new Error('Usage requires --input-format stream-json')
        }
        return await this.request({ subtype: 'get_usage' }, this.childStdin)
    }
...
export function query(config: {
    ...
    child.on('close', (code) => {
        if (config.options?.abort?.aborted) {
            ...
            const err = new Error(`Claude Code process exited with code ${code}: ${detail}`)
            query.setError(err)
            rejectExit(err)
        } else {
            resolveExit()
